...
const saveContentItem = async ({contentItem, languageCode}) => {
	...
	if (contentItem.properties.state === 3) {
		...
		const currentItem = await store.getItem({ options, itemType: "item", languageCode, itemID: contentItem.contentID });
		if (currentItem) {

			referenceName = currentItem.properties.referenceName;
			definitionName = currentItem.properties.definitionName;

			await store.deleteItem({ options, itemType: "item", languageCode, itemID: contentItem.contentID });
		}


	} else {
		//regular item
		await store.saveItem({ options, item: contentItem, itemType: "item", languageCode, itemID: contentItem.contentID });
	}

	if (referenceName) {
		//save the item by reference name - it might need to be merged into a list
		await store.mergeItemToList({ options, item: contentItem, languageCode, itemID: contentItem.contentID, referenceName, definitionName });
	}
}
...
const savePageItem = async ({pageItem, languageCode}) =>  {
	...
	if (pageItem.properties.state === 3) {
		//item is deleted
		await store.deleteItem({ options, itemType: "page", languageCode, itemID: pageItem.pageID });
	} else {
		//regular item
		await store.saveItem({ options, item: pageItem, itemType: "page", languageCode, itemID: pageItem.pageID });
	}
}
	...
	if (depth > 0) {
		...
		for (const zoneName in pageItem.zones) {
			...
			for (const mod of zone) {
				const moduleItem = await getContentItem({ options, contentID: mod.item.contentid, languageCode, depth: depth - 1 });
				mod.item = moduleItem;
			}
		}
